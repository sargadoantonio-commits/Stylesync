...
exports.syncServerPasswordCredential = exports.signInWithUsernameSecure = exports.registerWithUsernameSecure = void 0;
...
function validatePasswordSyncData(data) {
    const password = String(data.password ?? "");
    if (password.length < 8 || password.length > 100) {
        return { isValid: false, error: "Password must be 8-100 characters long." };
    }
...
async function jitterFailedAttempt() {
    await sleepMs(180 + Math.floor(Math.random() * 420));
}
function isBcryptDigest(hash) {
    return !!hash && hash.startsWith("$2");
}
async function digestMatches(password, credentialSalt, storedDigest, serverPepper) {
    const material = password + credentialSalt + serverPepper;
    if (isBcryptDigest(storedDigest)) {
        try {
            return await bcrypt_1.default.compare(material, storedDigest);
        }
        catch {
            return false;
        }
...
async function writeBcryptCredential(uid, password, serverPepper) {
    const credentialSalt = randomSalt();
    const material = password + credentialSalt + serverPepper;
    const passwordDigest = await bcrypt_1.default.hash(material, BCRYPT_COST);
...
async function tryIdentityToolkitSignIn(email, password) {
...
async function stripLegacyRootCredentials(uid) {
    const ref = (0, firestore_1.getFirestore)().collection("users").doc(uid);
    ...
    if (d.passwordHash != null || d.passwordSalt != null) {
        await ref.update({
            passwordHash: firestore_1.FieldValue.delete(),
            passwordSalt: firestore_1.FieldValue.delete(),
            updatedAt: firestore_1.FieldValue.serverTimestamp(),
        });
    }
}
...
const REG_MAX = 8;
function rateDocId(prefix, raw) {
    return `${prefix}_${sha256Hex(raw).slice(0, 48)}`;
}
async function assertUnderLoginFailCap(nu) {
    const db = (0, firestore_1.getFirestore)();
    ...
    if (now - start.seconds > LOGIN_FAIL_WINDOW_SEC)
        return;
    if (Number(d.failCount ?? 0) >= LOGIN_FAIL_MAX) {
        throw new https_1.HttpsError("resource-exhausted", "Too many sign-in attempts. Try again later.");
    }
}
async function recordLoginFailure(nu) {
    const db = (0, firestore_1.getFirestore)();
    const ref = db.collection(RATE_COL).doc(rateDocId("login_fail", nu));
    await db.runTransaction(async (tx) => {
        const snap = await tx.get(ref);
        const nowTs = firestore_1.Timestamp.now();
        const updatedAt = firestore_1.FieldValue.serverTimestamp();
        if (!snap.exists) {
...
async function clearLoginFailures(nu) {
    try {
        await (0, firestore_1.getFirestore)().collection(RATE_COL).doc(rateDocId("login_fail", nu)).delete();
    }
    catch {
        /* ignore */
    }
}
async function assertRegisterRateOk(emailLower) {
    const db = (0, firestore_1.getFirestore)();
    ...
    await db.runTransaction(async (tx) => {
        ...
        if (c > REG_MAX) {
            throw new https_1.HttpsError("resource-exhausted", "Too many registration attempts. Try again later.");
        }
        tx.update(ref, { count: c, updatedAt });
    });
}
exports.registerWithUsernameSecure = (0, https_1.onCall)({
    timeoutSeconds: 60,
    memory: "512MiB",
    ...
    if (!serverPepper || serverPepper.length < 16) {
        ...
        throw new https_1.HttpsError("failed-precondition", "Server configuration error. Please contact support.");
    }
    ...
    if (indexSnap.exists) {
        ...
        throw new https_1.HttpsError("already-exists", "This username is already taken. Please choose another one.");
    }
    ...
    try {
        ...
        throw new https_1.HttpsError("internal", "Registration failed. Please try again later.");
    }
    const credentialSalt = randomSalt();
    const material = password + credentialSalt + serverPepper;
    const passwordDigest = await bcrypt_1.default.hash(material, BCRYPT_COST);
...
exports.signInWithUsernameSecure = (0, https_1.onCall)({
    timeoutSeconds: 30,
    memory: "512MiB",
    enforceAppCheck: true,
}, async (request) => {
    ...
    if (!serverPepper || serverPepper.length < 16) {
        ...
        throw new https_1.HttpsError("failed-precondition", "Service temporarily unavailable. Please try again later.");
    }
    const data = (request.data ?? {});
    ...
    if (!indexSnap.exists) {
        ...
        throw new https_1.HttpsError("permission-denied", GENERIC_LOGIN);
    }
...
exports.syncServerPasswordCredential = (0, https_1.onCall)({
    timeoutSeconds: 30,
    memory: "256MiB",
    enforceAppCheck: true,
}, async (request) => {
    ...
    if (!uid) {
        ...
        throw new https_1.HttpsError("unauthenticated", "Please sign in to continue.");
    }
    ...
    try {
        ...
        throw new https_1.HttpsError("internal", "Failed to update password. Please try again later.");
    }
});
